...
{
    // Get the web page HTML
    NSString *data_str = url;
    
    // prepare regular expression to find text
    NSError *error   = nil;
    NSRegularExpression *regexp =
    [NSRegularExpression regularExpressionWithPattern:
//        @"^http://(b.vimeocdn.com/.+.jpg|.*.smilevideo.jp/smile\?i=.{8})" options:0 error:&error];
        @"^http://b.vimeocdn.com/.+.jpg" options:0 error:&error];
    // find by regular expression
    NSTextCheckingResult *match =
    [regexp firstMatchInString:data_str options:0 range:NSMakeRange(0, data_str.length)];
    ...
    if (match) {
        ...
        // get the og:image URL from the find result
        NSRange urlRange = NSMakeRange(resultRange.location, resultRange.length - 1);
